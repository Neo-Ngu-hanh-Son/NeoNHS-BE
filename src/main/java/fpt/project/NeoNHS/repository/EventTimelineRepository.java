...
public interface EventTimelineRepository extends JpaRepository<EventTimeline, UUID>, JpaSpecificationExecutor<EventTimeline> {
    ...
    List<EventTimeline> findByEventIdAndDate(UUID eventId, LocalDate date);
    List<EventTimeline> findByEventIdOrderByDateAscStartTimeAsc(UUID eventId);
    List<EventTimeline> findByEventPointId(UUID eventPointId);
}
