...
public class DataInitializer implements CommandLineRunner {
    ...
    // ABOUT_NHS:'About NHS',KNOW_BEFORE_YOU_GO:'Know before you go',
    // }as const;
    private void initializeBlogs() {
        if (blogCategoryRepository.count() > 0) {
            return;
        }
        List<BlogCategory> blogCategories = Arrays.asList(
                BlogCategory.builder()
                        .name("About NHS")
                        .description("Hiện ở trang homepage phần about NHS (max 2)")
                        .build(),
                BlogCategory.builder()
                        .name("Know before you go")
                        .description("Hiển thị trên homepage mobile mục hướng dẫn (Know before you go) (max 4)")
                        .build());
